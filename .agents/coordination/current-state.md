...
- **QR 임시 사진 전달·iPad 대응 설계 승인 (2026-08-13)**: 완성 결과 카드 PNG만 Cloudflare Worker+비공개 R2에 10분간 저장하고, QR(`receive.html#token`)으로 휴대폰에 전달한다. 원본 카메라·타깃·스냅샷은 전송하지 않는다. 설계: `docs/superpowers/specs/2026-08-13-qr-photo-transfer-design.md`. 구현 계획·코드는 사용자 설계 문서 검토 후 진행.
