...
Last updated: 2026-06-14 22:50 KST by Codex
...
- **System B (실시간 웹캠 감정 기록) 완료 (브라우저 검증됨 2026-06-08, 화면색 반영 제거 2026-06-14)**: `@vladmandic/face-api` + `public/models/` 가중치, `src/emotionMapping.js`(순수 매핑)·`src/faceEmotion.js`·`src/liveEmotion.js`. 관람자 실시간 감정은 결과 집계와 모자이크 재료에만 쓰고, 화면 틴트(`#tint`)는 항상 투명하게 유지해 영상/화면을 원본 색으로 보여준다.
