...
- **QR 임시 사진 전달·iPad 대응 구현 완료, 통합 대기 (2026-08-13)**: `codex/qr-photo-transfer` worktree에 최종 결과 카드 PNG만 Cloudflare Worker+비공개 R2로 전달하는 QR(`receive.html#token`) 기능, iPad Web Share/download fallback·safe area UI, 다중 Pages 빌드·배포 설정을 구현했다. 원본 카메라·타깃·스냅샷은 전송하지 않는다. `npm test` 184개, `npm run build`, Worker dry-run, 로컬 Worker PNG 왕복이 통과했으며 사용자 통합 선택을 기다린다. 설계: `docs/superpowers/specs/2026-08-13-qr-photo-transfer-design.md`; 계획: `docs/superpowers/plans/2026-08-13-qr-photo-transfer.md`.
